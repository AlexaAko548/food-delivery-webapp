          ...
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden z-10">
            ...
            <div className="p-6">
              ...
              <p className="text-sm text-gray-500 mb-6 leading-relaxed">
                {popupItem.description || "No description available."}
              </p>
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-3 bg-[#F8F5F1] rounded-2xl px-4 py-2">
                  <button
                    onClick={() => setPopupQty((q) => Math.max(1, q - 1))}
                    className="w-7 h-7 rounded-full bg-[#EAE0D5] text-[#5c4033] font-bold hover:bg-[#d4c5b2] transition flex items-center justify-center"
                  >−</button>
                  <span className="font-bold text-[#5c4033] w-5 text-center">{popupQty}</span>
                  <button
                    onClick={() => setPopupQty((q) => q + 1)}
                    className="w-7 h-7 rounded-full bg-[#5c4033] text-white font-bold hover:bg-[#3e2c22] transition flex items-center justify-center"
                  >+</button>
                </div>
                <button
...
function FoodCard({
  ...
  return (
    <div
      onClick={onOpen}
      className="h-full bg-white rounded-2xl overflow-hidden shadow-sm border border-[#D4C8BA] flex flex-col hover:shadow-md transition-shadow cursor-pointer relative"
    >
      {badge && (
        <span className="absolute top-3 left-3 z-10 bg-[#997855] text-white text-xs font-bold px-2 py-1 rounded-full">
          {badge}
        </span>
      )}
      <div className="h-48 bg-[#EAE0D5] shrink-0 relative overflow-hidden">
        {item.imageURL ? (
          ...
          <div className="w-full h-full flex items-center justify-center text-[#997855]/50 font-medium">
            image
          </div>
        )}
      </div>
